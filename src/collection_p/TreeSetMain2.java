...
class TreeStud1 implements Comparable{
	//type : 일반(국,영,수), 특기생(국,영,수,예체능)  ,  반,   이름,  총점,   점수,   등급(수우미양가 -> 점수기준)
	String type, name, grade, gender, cutStr;
	int ban, tot, res, rank;
	int [] jum;
	public TreeStud1(String name, String gender, int ban, int kor, int eng, int mat) {
		this("일 반", ban,gender, name, kor,  eng,  mat);
	}
	
	public TreeStud1(String name, String gender, int ban, int kor, int eng, int mat, int art) {
		this("특기생", ban,gender, name, kor,  eng,  mat, art);
	}
	
	private TreeStud1(String type, int ban, String gender, String name, int ...jum) {
		super();
		this.type = type;
		this.gender = gender;
		this.ban = ban;
	...
	void rankCalc(int no, ArrayList arr) {
		...
		for (Object object : arr) {
			...
			TreeStud1 you = (TreeStud1)object;
			boolean [] boArr = {
...
public class TreeSetMain2 {
	...
	public static void main(String[] args) {
		...
		dataArr.add(new TreeStud1("이유빈", "여", 2, 90, 95, 75));
		dataArr.add(new TreeStud1("김보름", "여", 2, 90, 75, 80));
		dataArr.add(new TreeStud1("이유빈", "남", 2, 65, 85, 80));
		dataArr.add(new TreeStud1("박성현", "남", 1, 85, 85, 60));
		dataArr.add(new TreeStud1("이승훈", "여", 1, 75, 55, 74));
		dataArr.add(new TreeStud1("이승훈", "남", 1, 55, 40, 75));
		dataArr.add(new TreeStud1("곽윤기", "남", 1, 90, 100, 85, 95));
		dataArr.add(new TreeStud1("서휘민", "여", 2, 75, 75, 60, 90));
		dataArr.add(new TreeStud1("이유빈", "남", 2, 70, 60, 70, 80));
		dataArr.add(new TreeStud1("최민정", "여", 1, 85, 80, 40, 65));
		dataArr.add(new TreeStud1("황대헌", "남", 1, 60, 75, 40, 85));
		dataArr.add(new TreeStud1("이유빈", "여", 2, 50, 50, 45, 75));
		/*
		String [] title = {
		...
		for (Object obj : ts0) {
			((TreeStud1)obj).rankCalc(0, dataArr);
			System.out.println(obj);
		}
		/*
